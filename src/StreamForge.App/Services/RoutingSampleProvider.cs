...
public sealed class RoutingSampleProvider : ISampleProvider
{
    ...
    public RoutingSampleProvider(ISampleProvider source, string mode, string outputMode = "Stereo")
    {
        _source = source; _mode = mode; _inputChannels = source.WaveFormat.Channels;
        var outputChannels = outputMode.Equals("Mono", StringComparison.OrdinalIgnoreCase) || mode == "Mono (L+R)" ? 1 : 2;
    ...
    public int Read(float[] buffer, int offset, int count)
    {
        ...
        for (var f = 0; f < inputFrames; f++) { var l = _inputBuffer[f * _inputChannels]; var r = _inputChannels > 1 ? _inputBuffer[f * _inputChannels + 1] : l; if (outputChannels == 1) buffer[offset + f] = _mode == "Solo sinistro" ? l : _mode == "Solo destro" ? r : (l + r) * .5f; else { var selected = _mode == "Solo sinistro" ? l : _mode == "Solo destro" ? r : 0; buffer[offset + f * 2] = _mode.StartsWith("Solo") ? selected : l; buffer[offset + f * 2 + 1] = _mode.StartsWith("Solo") ? selected : r; } }
